...
export default defineNitroPlugin(async (nitro) => {
	...
	await startTelegram(
			...
			bot.on("inline_query", async (ctx) => {
				try {
					...
					const getPhotoUrl = (userId: string, userPhotoUrl?: string) => {
						...
						const url = new URL(
							`/api/message/user-photo/${encodeURIComponent(userId)}.webp`,
							appUrl,
						);
					...
					for (const [index, request] of inlineQueries
						.slice(offsetStep)
						.entries()) {
						...
						const transformedResult = Array.isArray(awaitedResult)
							? awaitedResult
							: [awaitedResult];
						results.push(...transformedResult);
						if (results.length >= 50) {
							const currentOffset = index
								? transformedResult.length
								: offset + transformedResult.length;
		...
		{
			restart:
				typeof restartTelegram === "string"
					? restartTelegram === "true"
					: restartTelegram,
			restartInterval: restartTelegramInterval
				? Number(restartTelegramInterval)
				: undefined,
		},
	);
});
