...
export default defineMeasurementsInlineQuery({
	...
	selfArticle: {
		...
		description: () =>
			`Надіслати свої заміри за: ${toZonedTime(new Date(), "Europe/Kyiv").toLocaleDateString("uk-UA")}`,
	...
	managedArticle: {
		...
		description: () =>
			`Надіслати заміри за: ${toZonedTime(new Date(), "Europe/Kyiv").toLocaleDateString("uk-UA")}`,
		thumbnail_url: (user, { getPhotoUrl }) =>
			getPhotoUrl(user._id.toString(), user.photoUrl),
	},
});
