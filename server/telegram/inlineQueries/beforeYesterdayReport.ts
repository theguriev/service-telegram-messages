...
export default defineReportInlineQuery({
  ...
  selfArticle: {
    ...
    description: () => `Надіслати свій звіт за: ${toZonedTime(subDays(new Date(), 2), "Europe/Kyiv").toLocaleDateString("uk-UA")}`,
  ...
  managedArticle: {
    ...
    description: () => `Надіслати звіт за: ${toZonedTime(subDays(new Date(), 2), "Europe/Kyiv").toLocaleDateString("uk-UA")}`,
    thumbnail_url: (user, { getPhotoUrl }) => getPhotoUrl(user._id.toString(), user.photoUrl),
  },
});
