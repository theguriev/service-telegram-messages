...
export default defineReportInlineQuery({
  ...
  selfArticle: {
    ...
    description: () => `Надіслати свій звіт за: ${toZonedTime(new Date(), "Europe/Kyiv").toLocaleDateString("uk-UA")}`,
  ...
  managedArticle: {
    ...
    description: () => `Надіслати звіт за: ${toZonedTime(new Date(), "Europe/Kyiv").toLocaleDateString("uk-UA")}`,
    thumbnail_url: (user, { getPhotoUrl }) => getPhotoUrl(user._id.toString(), user.photoUrl),
  },
});
