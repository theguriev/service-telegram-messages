...
type ManagedQueryParams<TWord extends string> = Parameters<
	typeof defineManagedInlineQuery<TWord, MeasurementsUser>
>[0];
type MeasurementQueryParams<TWord extends string> = Omit<
...
const valueIndicators: {
	...
	},
	{
		...
		callback: ({ value, previousValue, startValue, goal }, key) => {
			...
			const changeValue = new Big(previousValue ?? 0)
				.minus(value)
				.abs()
				.toNumber();
...
const defineMeasurementsInlineQuery = <TWord extends string>(
	params: MeasurementQueryParams<TWord>,
) =>
