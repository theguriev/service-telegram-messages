...
export default defineTask({
	meta: {
		...
		description:
			"Sends notifications to managers with users, that didn't send reports",
	},
	async run() {
		...
		for (const { managerId, users } of result) {
				...
				try {
					...
					if (balance) {
						...
						const daysDifference = message
							? differenceInDays(
									startOfDay(new Date()),
									startOfDay(message.createdAt),
								)
							: differenceInDays(
									startOfDay(new Date()),
									startOfDay(user.createdAt),
								);

						if (daysDifference < 1) return "";
						const daysString = plural(
							daysDifference,
							"%d день",
							"%d дні",
							"%d днів",
			...
			const userLinks = (await Promise.all(userLinksAsync)).filter(Boolean) as {
				text: string;
				url: string;
			}[];
